...
@section('css')
    ...
    <link rel="stylesheet" type="text/css" href="{{ config('voyager.assets_path') }}/css/datepicker3.css">
                            ...
                            @foreach($dataTypeRows as $row)
                                <div class="form-group">
                                    ...
                                    @if($row->type == "text")
                                        ...
                                        @if(  isset($dataTypeContent->{$row->field}) &&  isset($options->dateformat) )
                                            <?php
                                            $dataTypeContent->{$row->field} = 
                                            \Carbon\Carbon::parse($dataTypeContent->{$row->field})->format($options->dateformat);
                                            ?>
                                        @endif
...
@section('javascript')
    <script>
        ...
        $('document').ready(function () {
            ...
            $('[name="main_contractor_awarded_date"], [name="mep_contractor_awarded_date"], [name="construction_start_date"], [name="construction_end_date"]').datepicker({
                format: "dd-M-yyyy",
                todayBtn: "linked"
            });
            ...
            //decimals
            $('[name="value"], [name="map_latitude"], [name="map_longitude"]').on("keydown", function() {
                pastValue          = this.value;
                pastSelectionStart = this.selectionStart;
                pastSelectionEnd   = this.selectionEnd;
            }).on("input propertychange", function() {
                                
                if ( $.isNumeric( this.value) == false && this.value.length > 0 && this.value != '-' ) {
            ...
            //integers
            $('[name="num_of_floors"], [name="num_of_rooms"]').on("keydown", function() {
                pastValue          = this.value;
                pastSelectionStart = this.selectionStart;
                pastSelectionEnd   = this.selectionEnd;
            }).on("input propertychange", function() {
                                
                if ( this.value != parseInt(this.value, 10) && this.value.length > 0 ) {
                    this.value          = pastValue;
                    this.selectionStart = pastSelectionStart;
                    this.selectionEnd   = pastSelectionEnd;
                }
    ...
    <script src="{{ config('voyager.assets_path') }}/js/bootstrap-datepicker.js"></script>
@stop
