        ...
        foreach ($rows as $row) {
            ...
            if (isset($options->validation)) {
                if (isset($options->validation->rule)) {
                    ...
                    //remove required validation from image field when editing record
                    if( isset( $data->id ) && ($row->type == 'image' || $row->type == 'file' ) )
                    {
                        $options->validation->rule = str_replace('required', '', $options->validation->rule);
                    }
                    //edit - remove required validation from image field when editing record
                    ...
                    if( strpos(strtoupper($options->validation->rule), 'UNIQUE') !== false )
                    {
                        ...
                        $explodedRules[] = \Illuminate\Validation\Rule::unique($slug)->ignore($data->id);
        ...
        foreach ($rows as $row) {
            ...
            if (isset($options->validation)) {
                if (isset($options->validation->rule)) {
                    ...
                    if( strpos(strtoupper($options->validation->rule), 'DATE') !== false )
                    {
                        if($data->{$row->field} != '')
                            $data->{$row->field} = \Carbon\Carbon::parse( $data->{$row->field} )->format('Y-m-d H:i:s');
                        else
                            $data->{$row->field} = null;
                    }
                }
